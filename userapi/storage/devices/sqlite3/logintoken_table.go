...
func (s *loginTokenStatements) prepare(db *sql.DB) error {
	return sqlutil.StatementList{
		...
		{&s.deleteStmt, "DELETE FROM login_tokens WHERE token = $1 OR token_expires_at <= CURRENT_TIMESTAMP"},
		{&s.selectStmt, "SELECT user_id FROM login_tokens WHERE token = $1 AND token_expires_at > CURRENT_TIMESTAMP"},
	}.Prepare(db)
}
...
func (s *loginTokenStatements) selectByToken(ctx context.Context, token string) (*api.LoginTokenData, error) {
	...
	err := s.selectStmt.QueryRowContext(ctx, token).Scan(&data.UserID)
	if err != nil {
		return nil, err
	}
